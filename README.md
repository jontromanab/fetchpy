...
The arm and torso combined also makes a 8DOF manipulator where 7DOF is associated with the arm and 1 extra DOF is from the prismatic joint of the torso. The system calculates a 8DOF IKFast solution **(freeindex = 3,4)** for planning with arm and torso. All the valid commands for arm manipulation can also be executed on this manipulator too.  e.g. To plan and execute a trajectory defined by joint configuration:
```
...
The whole body controller is tailered to control arm, torso and base at the same time. For a simple example, to move the whole body by configuration(the last two values are robot base linear and angular velocity:
```
angle = ([0.3, 0.35, -0.5,0.5,0.75,-0.55,0.78,0.9, 0.2, 0.1]) 
robot.whole_body.PlanToConfiguration(angle, execute = True) 
```
